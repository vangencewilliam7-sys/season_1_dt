...
# so it can find the 'app' module when run directly via 'python test_pii_scrubber.py'
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from app.services.pii_scrubber import PIIScrubber

# Ensure a stable key is used for testing
...
def test_medical_terms_preserved(scrubber):
    ...
    assert "MEDICAL_CONDITION" not in scrubbed # It shouldn't create a token
...
def test_key_mismatch():
    ...
    scrubber_a._fernet = scrubber_a._build_fernet("key_A_password")
    
    scrubber_b = PIIScrubber()
    scrubber_b._fernet = scrubber_b._build_fernet("key_B_password")
    
    text = "Hello Anu"
    scrubbed_a = scrubber_a.scrub(text)
    
    # Try restoring with scrubber B
